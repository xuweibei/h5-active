const position=[    //三维数组分别代表16个格子的位置: 不同行的x相等， y加109；同一行的y相等，x加109 用来定义单元格的位置
...
function operateEvent(rank = 0, dire = 0) {
  ...
  for (let i = 0; i < els.length; i++) {
    ...
    //findIndex方法返回满足条件的数组下标, index为满足条件的数组下标， needCharge是一个二维数组， element这个二维数组里面的一维数组
     //这个方法可以找到元素在列中的第几个位置，也就是第几行index,共四行,下标[0,3]
    let index = needCharge.findIndex((element, index, array) => {
      return element.toString() === nowPos.toString();
    });
    // TODO 融合操作，应该如何处理
    if (dire) { //dire ===1表示down和right事件
      // 如果是down和right事件(向下或向右)，需要下移或右移
    ...
    if (!rank) {   //上下
      ...
      els[i].style.top = position[index][nowPos[0]][1];   //上下改变的是y轴+109 ||-109, 改变列，所在行不变
    } else {   //左右
      els[i].dataset.pos = index + ',' + nowPos[1] + ',' + posArr[posArr.length - 1];  //左右改变的是x轴
      els[i].style.left = position[nowPos[1]][index][0];   //左右改变的是X轴+109 ||-109 左右改变列的位置，所在行不变
    }
...
window.onload = () => {
  ...
  document.onkeyup = (e) => {
    let code = e.keyCode;  //获取键盘的对应的number值 键盘上37为左键，38为上键，39为右键，40为下键
    console.log('code', code);
    let tileContainer = document.querySelector('.tile-container');  //获取包含单元格最外层的div
    let tiles = document.querySelectorAll('.tile');  //获取所有的单元格
    console.log('tiles3333', tiles);
    if (tiles.length === 16) {   // 如果单元格已经有16个，让每一个依次在上一个的180ms之后消失
        for (let i = 0; i<tiles.length; i++) {
            setTimeout(() => {
        ...
        setTimeout(() =>{
            tileContainer.innerHTML = '';  //清空所有的单元格
        }, 30000)
    } else {
      let flag = false;
      switch (code) {
