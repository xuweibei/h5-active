...
var createNum = function(num=2) {
  ...
  tile.style.top = position[pos[0]][pos[1]][0];   //单元格绝对定位的位置
  tile.style.left = position[pos[0]][pos[1]][1];
  console.log('tile.style.top', tile.style.top, 'tile.style.left', tile.style.left);
...
function operateEvent(rank = 0, dire = 0) {
  ...
  for (let i = 0; i < els.length; i++) {
    ...
    // TODO 融合操作，应该如何处理
    if (dire) {
      // 如果是down和right事件，并且这个元素和下一个元素的值相等，那么index++;
